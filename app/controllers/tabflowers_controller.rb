 ...
 if @tabuser!=nil
  ...
  def checkout
     ...
     if @tabflower_shop!=nil then
         ...
         @totalfp+=tabflower.current_price*amount
         @totalfs+=tabflower.origin_price*amount
         end
     end
     else
  ...
  def search
    searchName=params[:tabflower][:name]
    sql="select * from tabflowers where name like '%"+searchName+"%'"
